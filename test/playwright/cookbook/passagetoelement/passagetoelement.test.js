...
test.describe('passagetoelement Example', () => {
  test('should load without errors', async ({ page }) => {
    const { consoleErrors, pageErrors } = setupErrorTracking(page);
    
    await page.goto(`file://${compiledHtmlPath}`);
    // Wait for the #hudID div to be in DOM (even if empty initially)
    await page.waitForSelector('#hudID', { state: 'attached', timeout: 5000 });
    // Wait for content to be rendered into #hudID
    await page.waitForSelector('#hudID h1', { state: 'attached', timeout: 5000 });
  ...
  test('should render passage element with correct structure', async ({ page }) => {
    ...
    await page.goto(`file://${compiledHtmlPath}`);
    // Wait for #hudID div first
    await page.waitForSelector('#hudID', { state: 'attached', timeout: 5000 });
    // Wait for h1 to be rendered inside hudID
    await page.waitForSelector('#hudID h1', { state: 'attached', timeout: 5000 });
    
    logErrors(consoleErrors, pageErrors);
    
    await expect(page.locator('tw-storydata')).toBeAttached();
    await expect(page.locator('#hudID h1')).toHaveText('This is the HUD!');
