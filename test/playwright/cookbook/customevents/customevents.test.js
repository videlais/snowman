...
const compiledHtmlPath = join(__dirname, 'snowman_customevents.html');
...
test.describe('Custom Events for Notifications Example', () => {
  ...
  test('should show an achievement toast when the custom event fires', async ({ page }) => {
    const { consoleErrors, pageErrors } = setupErrorTracking(page);
    ...
    await expect(toast).toContainText('Achievement unlocked: Sword Collector');

    expect(pageErrors).toHaveLength(0);
    expect(consoleErrors).toHaveLength(0);
  });

  test('should remove the toast automatically after a short delay', async ({ page }) => {
    await page.goto(`file://${compiledHtmlPath}`);
    await page.waitForSelector('tw-passage', { timeout: 5000 });

    await page.click('#unlock-btn');
    await expect(page.locator('.achievement-toast')).toHaveCount(1);
    await expect(page.locator('.achievement-toast')).toHaveCount(0, { timeout: 3000 });
  });
});
