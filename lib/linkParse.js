...
export function parse (text) {
    ...
    // Regex to match Twine-style links. Name/destination chars allow "-" unless it starts a
    // "->" delimiter, and treat unevaluated "<%...%>" template blocks as literal text so that
    // links containing templates (rendered later) still match.
    const linkRegex = /\[\[((?:<%[\s\S]*?%>|[^\]|<-]|-(?!>))+)(?:\|((?:<%[\s\S]*?%>|[^\]|<-]|-(?!>))+)|->((?:<%[\s\S]*?%>|[^\]|<-]|-(?!>))+)|<-((?:<%[\s\S]*?%>|[^\]|<-]|-(?!>))+))?\]\]/g;
    ...
    text = text.replace(linkRegex, (match, p1, p2, p3, p4) => {
      ...
      if (p1 && p4 && p2 === undefined && p3 === undefined) {
        return `<a href="javascript:void(0)" data-passage="${p1}">${p4}</a>`;
      }
