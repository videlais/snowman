...
export default class Passage {
  ...
  render(source) {
    ...
    // 3. Process markdown (template blocks are protected so their JS is never touched)
    ...
    if (!hasTemplates) {
      ...
      result = result.replace(htmlTag, function(match, tag, attrs, content) {
        ...
        if (attrs.includes('=')) {
          ...
          // List of known HTML boolean attributes that should be preserved as-is
          const knownBooleanAttrs = ['controls', 'autoplay', 'loop', 'muted', 'disabled', 
            'readonly', 'required', 'checked', 'selected', 'multiple', 'autofocus', 
            'formnovalidate', 'novalidate', 'async', 'defer'];
          ...
          for (const part of parts) {
            ...
            } else if (knownBooleanAttrs.includes(part.toLowerCase())) {
              booleanAttrs.push(part);
            } else {
          ...
          // Combine all attributes: standard + boolean + processed shorthands
          let allAttrs = standardAttrs.concat(booleanAttrs).join(' ');
          ...
          // No standard attributes with = sign
          // Check if this might be a boolean attribute or shorthand
          const knownBooleanAttrs = ['controls', 'autoplay', 'loop', 'muted', 'disabled', 
            'readonly', 'required', 'checked', 'selected', 'multiple', 'autofocus', 
            'formnovalidate', 'novalidate', 'async', 'defer'];
          ...
          for (const part of parts) {
            if (knownBooleanAttrs.includes(part.toLowerCase())) {
              booleanAttrs.push(part);
            } else {
              shorthandParts.push(part);
            }
          ...
          let allAttrs = booleanAttrs.join(' ');
          
          if (shorthandParts.length > 0) {
            const processedAttrs = renderAttrs(shorthandParts.join(' ')).trim();
            if (processedAttrs) {
              allAttrs += (allAttrs ? ' ' : '') + processedAttrs;
            }
          }
    ...
    // Step 3: Process markdown (template blocks are protected from interference)
  ...
  /**
   * Replaces each Underscore template block (<%, <%=, <%-  ...  %>) with an opaque
   * placeholder token so markdown parsing can't misinterpret or HTML-escape JS code.
   * @param {string} text - The text to scan for template blocks
   * @returns {{text: string, blocks: string[]}} Protected text and the original blocks
   */
  protectTemplateBlocks(text) {
    const blocks = [];
    // Non-greedy match across all three template forms: <%, <%=, <%-
    const templateRe = /<%[-=]?[\s\S]*?%>/g;
    const protectedText = text.replace(templateRe, (match) => {
      // Private Use Area characters have no markdown/HTML meaning, so they pass through untouched
      const token = `\uE000TPL${blocks.length}\uE000`;
      blocks.push(match);
      return token;
    });
  ...
  /**
   * Reverses protectTemplateBlocks(), restoring the original template code verbatim.
   * @param {string} text - Text containing placeholder tokens
   * @param {string[]} blocks - Original template blocks, indexed by protectTemplateBlocks()
   * @returns {string} Text with placeholders replaced by their original template blocks
   */
  restoreTemplateBlocks(text, blocks) {
   ...
   * 
   * Template blocks are protected before markdown runs (and restored after) so that
   * JS code containing <, >, *, _, or backticks is never mangled or HTML-escaped.
   * 
   * @param {string} text - The text to process
   * @returns {string} Processed HTML
   */
  processMarkdown(text) {
    const { text: protectedText, blocks } = this.protectTemplateBlocks(text);

    // Check if text contains block-level markdown elements
    const hasHeaders = /^#{1,6}\s/m.test(protectedText);
    const hasLists = /^[-*+]\s/m.test(protectedText) || /^\d+\.\s/m.test(protectedText);
    const hasBlockquotes = /^>\s/m.test(protectedText);
    const hasCodeBlocks = protectedText.includes('```');
    const hasHorizontalRules = /^---\s*$/m.test(protectedText) || /^\*\*\*\s*$/m.test(protectedText);
    ...
    const textWithoutHtml = protectedText.replace(/<[^>]+>/g, '');
    const hasInlineMarkdown = /[*_`]/.test(textWithoutHtml);

    // If no markdown syntax at all, return text as-is (restoring any template blocks)
    ...
    if (!hasHeaders && !hasLists && !hasBlockquotes && !hasCodeBlocks && !hasHorizontalRules && !hasInlineMarkdown) {
      return this.restoreTemplateBlocks(protectedText, blocks);
    }
    ...
    const renderedText = (hasHeaders || hasLists || hasBlockquotes || hasCodeBlocks || hasHorizontalRules)
      ? marked.parse(protectedText, markedOptions)
      : marked.parseInline(protectedText, markedOptions);

    return this.restoreTemplateBlocks(renderedText, blocks);
  }
