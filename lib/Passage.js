...
export default class Passage {
  ...
  render(source) {
    ...
    const hasTemplates = /<[%][\s\S]*?[%]>/.test(source);
    ...
    if (!hasTemplates) {
      const htmlTag = /<([a-zA-Z][-a-zA-Z0-9]*)(.*?)>(.*?)<\/\1>/g;
    ...
    // Create a new regex to avoid state issues with the global flag
    const htmlTag2 = /<([a-zA-Z][-a-zA-Z0-9]*)(.*?)>(.*?)<\/\1>/g;
  ...
  processMarkdown(text) {
    ...
    // Configure marked to preserve HTML and match markdown-it behavior
    ...
    return marked.parseInline(text, { async: false, mangle: false });
  }
