...
class Story {
	/**
	 * Safely processes script content to prevent HTML parser issues with </script> tags.
	 * This method escapes problematic </script> sequences that would be interpreted
	 * by the HTML parser as the end of a script tag, even when inside strings or comments.
	 * 
	 * @param {string} scriptContent - The raw script content to process
	 * @returns {string} - The processed script content with escaped </script> tags
	 */
	static sanitizeScriptContent(scriptContent) {
		if (typeof scriptContent !== 'string') {
			return scriptContent;
		}
		
		// Replace </script> with <\/script> to prevent HTML parser issues
		// This works in string literals, comments, and other contexts
		// The backslash escapes the forward slash, making it safe for HTML parsing
		// while still being valid JavaScript when executed
	...
	constructor() {
		...
		this.userScripts = [];
     	...
     	// Apply sanitization to prevent HTML parser issues with </script> tags
     	for(let script of document.querySelectorAll('*[type="text/twine-javascript"]') ) {
			const rawContent = $(script).html();
			const sanitizedContent = Story.sanitizeScriptContent(rawContent);
			this.userScripts.push(sanitizedContent);
		}
